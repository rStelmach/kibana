...
describe('getStepDescription', () => {
  ...
  it('falls back to auto description when description is empty', () => {
    const step = asStep({
      action: 'set',
      to: 'field',
      value: 'bar',
  ...
  it('generates description for set processor with copy_from', () => {
    ...
    expect(getStepDescription(step)).toBe(
      'Sets value of "log.level" to value of "severity"'
    );
  });
  ...
  it('generates description for convert processor without target', () => {
    ...
    expect(getStepDescription(step)).toBe(
      'Converts "bytes" field value to "long" type'
    );
  });
